...
def imshow_det_bboxes(img,
    ...
    elif(show_type == 'Male'):  # if above_16 show green boxes with green text
        bbox_color = color_val("blue")
        text_color = color_val("blue")
    else:
...
def classify(data_model):
    ...
    for i in result:
        if (i[0] == 1):
            detected['Female'] = detected['Female'] + 1
            GLOBAL_NON_ADULT_INDEX.append(counter)
        else:
            detected['Male'] = detected['Male'] + 1
...
print("Pedestrain Classification Model has detected ",
      final_result['Female'], " Female(s) ", final_result['Male'], " Male(s)")
print('\n')
...
if(len(GLOBAL_ADULT_INDEX) > 0):
    ...
    show_mask_result(image, adult_result, 'result1.jpg',
                     showbox=True, show_type="Male")
...
if(len(GLOBAL_NON_ADULT_INDEX) > 0):
    ...
    show_mask_result(image, non_adult_result, 'result1.jpg',
                     showbox=True, show_type='Female')  # the bounding box is drawn
